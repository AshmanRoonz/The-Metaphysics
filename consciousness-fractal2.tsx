...
const ConsciousnessFractal = () => {
    ...
    const memorySystemRef = useRef({
        ...
        fluidCount: 0, // Track fluid particles
        membraneFortification: 0,
        membraneSealed: false
    });
    ...
    useEffect(() => {
        ...
        // FLUID SYSTEM: Internal circulation
        let fluidParticles = [];
        let fluidSystemActive = false;
        const MAX_FLUID_PARTICLES = 150;
        
        // MEMBRANE: Track fortification and sealing
        let membraneFortification = 0;
        let membraneSealed = false;
        let sealEventTime = null;
        ...
        // FLUID SYSTEM: Particle class for internal circulation
        class FluidParticle {
            constructor() {
                // Spawn within membrane boundary
                const spawnAngle = Math.random() * Math.PI * 2;
                const spawnRadius = Math.random() * 150; // Inside the membrane
                ...
                this.hue = 200 + Math.random() * 80; // Blue-cyan-purple range
                this.alpha = 0.3 + Math.random() * 0.4;
                this.life = 1.0;
                this.age = 0;
                
                this.phaseOffset = Math.random() * Math.PI * 2;
                this.flowSpeed = 0.3 + Math.random() * 0.4;
                
                // Trail history for organic flow visualization
                this.trail = [];
                this.maxTrailLength = 5;
            }
            
            update(time, brainClouds, validatedPatterns, fieldCoherence) {
                this.age++;
                ...
                // FLOW FIELD: Circular flow influenced by coherence
                const flowAngle = Math.atan2(dy, dx) + Math.PI / 2;
                const flowStrength = 0.2 * (1 + fieldCoherence * 0.5);
                
                // Add circular flow
                this.vx += Math.cos(flowAngle) * flowStrength * 0.05;
                this.vy += Math.sin(flowAngle) * flowStrength * 0.05;
                
                // ATTRACTION to nearby patterns (nutrients/energy sources)
                validatedPatterns.forEach(pattern => {
                    ...
                    if (dist < 60 && dist > 5) {
                        const attraction = 0.015 * (pattern.scale || 0.5);
                        this.vx += (pdx / dist) * attraction;
                        this.vy += (pdy / dist) * attraction;
                        
                        // Absorb pattern color slightly
                        this.hue = this.hue * 0.95 + pattern.hue * 0.05;
                    }
                });
                
                // REPULSION from membrane particles (containment)
                if (brainClouds.length > 0) {
                    brainClouds.forEach(cloud => {
                        cloud.particles.forEach(mp => {
                            const mpAngle = mp.angle + Math.sin(time * mp.speed + mp.phaseOffset) * 0.15;
                            const mpDist = cloud.distance + mp.distOffset;
                            const mpx = centerX + Math.cos(mpAngle) * mpDist;
                            const mpy = centerY + Math.sin(mpAngle) * mpDist;
                            
                            const mdx = this.x - mpx;
                            const mdy = this.y - mpy;
                            const mDist = Math.sqrt(mdx * mdx + mdy * mdy);
                            
                            if (mDist < 40) {
                                const repulsion = (40 - mDist) / 40 * 0.3;
                                this.vx += (mdx / mDist) * repulsion;
                                this.vy += (mdy / mDist) * repulsion;
                            }
                        });
                    });
                }
                
                // Boundary containment - strong force to stay inside
                if (distFromCenter > 160) {
                    const pushStrength = (distFromCenter - 160) / 160 * 0.5;
                    this.vx -= (dx / distFromCenter) * pushStrength;
                    this.vy -= (dy / distFromCenter) * pushStrength;
                }
                ...
                // Slow aging - fluid lives long
                this.life = Math.max(0, this.life - 0.001);
                
                // Pulsing alpha based on field coherence
                const pulse = Math.sin(time * 0.02 + this.phaseOffset) * 0.5 + 0.5;
                this.currentAlpha = this.alpha * this.life * (0.7 + pulse * 0.3 * fieldCoherence);
                
                return this.life > 0 && distFromCenter < 200;
            }
            
            draw(ctx) {
                // Draw subtle trail
                if (this.trail.length > 1) {
                    ctx.strokeStyle = `hsla(${this.hue}, 70%, 65%, ${this.currentAlpha * 0.3})`;
                ...
                ctx.shadowBlur = 6;
                ctx.shadowColor = `hsla(${this.hue}, 80%, 70%, ${this.currentAlpha * 0.6})`;
        ...
        class BrainCloud {
            constructor(angle, distance, birthTime) {
                ...
                for (let i = 0; i < 30; i++) {
                    this.particles.push({
                        ...
                        nestingPatterns: [],
                        // CRYSTAL GROWTH
                        crystalGrowth: 0, // 0 to 1, how much this particle has grown
                        growthRate: 0.001 + Math.random() * 0.001,
                        connectedNeighbors: [] // Which particles this is connected to
                    });
                }
            }
            ...
            draw(ctx, centerX, centerY, time, fieldCoherence, allPatterns) {
                ...
                this.particles.forEach(p => {
                    ...
                    // CRYSTAL GROWTH: Gradually grow particles over time
                    if (this.alpha > 0.5) { // Only grow when cloud is mature
                        p.crystalGrowth = Math.min(1, p.crystalGrowth + p.growthRate);
                    }
                    
                    // Calculate current size based on growth
                    const growthMultiplier = 1 + p.crystalGrowth * 3; // Grow up to 4x original size
                ...
                // CRYSTAL CONNECTIONS: Check which particles are touching/overlapping
                const connectionDistance = 15; // Distance to consider particles "connected"
                let totalPossibleConnections = 0;
                let actualConnections = 0;
                
                // Count how many particle pairs are connected (touching)
                // When crystals grow large enough to touch neighbors, gaps seal
                particlePositions.forEach((p1, i) => {
                    p1.particle.connectedNeighbors = [];
                    particlePositions.forEach((p2, j) => {
                        if (i < j) { // Only check each pair once
                            ...
                            // Particles are connected if they overlap or are very close
                            if (dist < combinedRadius + connectionDistance) {
                                actualConnections++;
                                p1.particle.connectedNeighbors.push(p2.particle);
                                p2.particle.connectedNeighbors.push(p1.particle);
                            }
                        }
                    });
                });
                
                // Fortification = how well connected the membrane is (gaps sealed)
                this.fortificationLevel = totalPossibleConnections > 0 ? actualConnections / totalPossibleConnections : 0;
        ...
        const drawEmergentFieldGlow = (ctx) => {
            ...
            // FORTIFICATION: Enhanced glow when membrane is sealed
            const sealBonus = membraneSealed ? 0.15 : 0;

            const gradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, glowRadius);
            gradient.addColorStop(0, `rgba(180, 200, 255, ${glowIntensity + sealBonus})`);
            gradient.addColorStop(0.5, `rgba(150, 170, 255, ${(glowIntensity + sealBonus) * 0.4})`);
            ...
            // SEALED: Draw containment field ring
            if (membraneSealed) {
                const ringRadius = 165 + Math.sin(time * 0.02) * 5;
                ctx.strokeStyle = `hsla(140, 80%, 70%, ${0.3 + Math.sin(time * 0.03) * 0.2})`;
                ctx.lineWidth = 2;
        ...
        function animate() {
            ...
            if (!brainFormed && checkBrainFormation()) {
                ...
                // FLUID: Activate internal circulation when membrane forms
                fluidSystemActive = true;
            }
            
            // FLUID SYSTEM: Spawn particles gradually as membrane strengthens and seals
            if (fluidSystemActive && brainClouds.length > 0) {
                const membraneStrength = brainClouds.reduce((sum, c) => sum + c.alpha, 0) / brainClouds.length;
                
                // Fluid production scales with both membrane strength AND fortification
                // Without sealed gaps, fluid leaks out - only fills when crystals connect
                const sealingBonus = membraneFortification * 0.5;
                const effectiveStrength = membraneStrength * (1 + sealingBonus);
                const targetFluidCount = Math.floor(MAX_FLUID_PARTICLES * effectiveStrength);
                
                // Spawn rate increases as membrane seals
                const spawnRate = membraneSealed ? 0.5 : 0.3;
                
                // Gradually add fluid particles
                if (fluidParticles.length < targetFluidCount && Math.random() < spawnRate) {
                ...
                brainClouds.forEach(cloud => {
                    if (Math.random() < 0.015) {
                        ...
                        if (Math.random() < 0.4) {
                            ...
                            validatedPatterns.push(new ValidatedPattern(cloud.angle + (Math.random() - 0.5) * 0.3, inputHue, time));
            ...
            // MEMBRANE: Calculate overall fortification level from particle crystallization
            if (brainClouds.length > 0) {
                const prevSealed = membraneSealed;
                membraneFortification = brainClouds.reduce((sum, cloud) => sum + cloud.fortificationLevel, 0) / brainClouds.length;
                membraneSealed = membraneFortification > 0.4; // Lower threshold since we measure actual connections
                
                // Trigger seal event
                if (membraneSealed && !prevSealed) {
                    sealEventTime = time;
                }
                
                memorySystemRef.current.membraneFortification = membraneFortification;
                memorySystemRef.current.membraneSealed = membraneSealed;
            }
            
            // FLUID SYSTEM: Update and filter fluid particles
            if (fluidSystemActive) {
                fluidParticles = fluidParticles.filter(particle => {
                    return particle.update(time, brainClouds, validatedPatterns, fieldCoherence);
                });
                memorySystemRef.current.fluidCount = fluidParticles.length;
            }

            drawEmergentFieldGlow(ctx);
            
            // SEAL EVENT: Celebration pulse when membrane first seals
            if (sealEventTime !== null && time - sealEventTime < 120) {
                const sealAge = time - sealEventTime;
                const pulseProgress = sealAge / 120;
                const pulseRadius = 50 + pulseProgress * 200;
                const pulseAlpha = (1 - pulseProgress) * 0.5;
                
                ctx.strokeStyle = `hsla(140, 90%, 75%, ${pulseAlpha})`;
                ...
                // Inner glow burst
                const burstGradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, pulseRadius * 0.5);
                burstGradient.addColorStop(0, `rgba(150, 255, 200, ${pulseAlpha * 0.8})`);
                burstGradient.addColorStop(1, 'rgba(150, 255, 200, 0)');
            ...
            // FLUID: Draw internal circulation (underneath other elements)
            if (fluidSystemActive) {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setMemoryStats({
                ...
                fluid: memorySystemRef.current.fluidCount,
                fortification: memorySystemRef.current.membraneFortification,
                sealed: memorySystemRef.current.membraneSealed
            });
        }, 1000);
            ...
            <div className="absolute top-8 right-8 text-gray-400 font-mono text-xs bg-gray-800/60 px-4 py-3 rounded-lg backdrop-blur-sm border border-cyan-600/30">
                ...
                <div className="space-y-1">
                    ...
                    <div className="flex justify-between gap-4 border-t border-gray-700/50 pt-1 mt-1">
                        <span className="text-gray-500">Membrane Status:</span>
                        <span className={memoryStats.sealed ? "text-green-300 font-semibold" : "text-orange-300 font-semibold"}>
                            {memoryStats.sealed ? "SEALED ✓" : `${(memoryStats.fortification * 100).toFixed(0)}%`}
                        </span>
                    </div>
                    <div className="flex justify-between gap-4">
                        <span className="text-gray-500">Internal Fluid:</span>
                        <span className="text-blue-300 font-semibold">{memoryStats.fluid}</span>
                    </div>
                </div>
                <div className="mt-3 pt-2 border-t border-gray-700/50 text-xs text-gray-500">
                    ...
                    <div className="text-blue-400">Fluid = circulation</div>
                    <div className="text-green-400">Crystals grow = seal</div>
                </div>
            </div>
        </div>
    );
